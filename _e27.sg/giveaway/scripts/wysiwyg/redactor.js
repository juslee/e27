...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
			...
			fonts: Array(
				'arial', 'arial black', 'comic sans ms', 'courier', 'courier new', 'georgia', 'helvetica', 'impact', 'Lucida Sans', 'Lucida Grande', 'palatino'
	...
	Redactor.prototype = {
		...
		getPath: function()
		{
			...
			$('script').each($.proxy(function(i,s)
			{
				if (s.src)
				{
					// Match redactor.js or redactor.js, followed by an optional querystring (often used for cache purposes)
		...
		pasteCleanUp: function(marker)
		{
			...
			$(this.doc.body).find('span[rel=pastemarkerend]').not('#pastemarkerend' + marker).remove();
		...
		buildToolbar: function()
		{
			...
			$.each(RTOOLBAR[this.opts.toolbar], $.proxy(function(key,s)
			{
				...
				// dropdown
				if (key == 'backcolor' || key == 'fontcolor' || key== 'fonttype' || typeof(s.dropdown) != 'undefined')
				{
					var dropdown = $('<div class="redactor_dropdown" style="display: none;">');

					if (key == 'backcolor' || key == 'fontcolor') dropdown = this.buildColorPicker(dropdown, key);
					else if(key == 'fonttype')  dropdown = this.buildFontPicker(dropdown, key);
		...
		buildFontPicker: function(dropdown, key)
		{
			var mode;
			...
			var len = this.opts.fonts.length;
			for (var i = 0; i < len; ++i)
			{
				var font = this.opts.fonts[i];
				
				var swatch = $('<a rel="' + font + '" href="javascript:void(null);">'+font+'</a>').css({ 'font-family': font });
				$(dropdown).append(swatch);
		...
		_imageSet: function(json, link)
		{
			if (link !== true)
			{
				...
				var html = '<p><img src="' + data.filelink + '" alt="' + data.filealt + '" /></p>';
			}
			else var html = json;
		...
		setupModalWindows: function()
		{
			...
			this.opts.modal = { 
				...
				'image_edit': '<label>' + RLANG.title + '</label><input id="redactor_file_alt" class="redactor_input" />'
						...
						+ '<div id="redactor_modal_footer"><input type="submit" class="button" value="delete" id="redactor_image_delete_btn" style="color: #000;"><span class="redactor_btns_box"><input type="button" name="save" id="redactorSaveBtn" value="' + RLANG.save + '" />&nbsp;&nbsp;<a href="javascript:void(null);" id="redactor_btn_modal_close">' + RLANG.cancel + '</a></span></div>',
