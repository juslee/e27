...
<head profile="http://gmpg.org/xfn/11">
	...
	<?php } ?>
    <?php
    //commented out to fix the repeating title in titlebar
    //bloginfo( 'name' );  $site_description = get_bloginfo( 'description', 'display' );
    ?>
	<title><?php global $page, $paged; wp_title( '|', true, 'right' );  if ( $site_description && ( is_home() || is_front_page() ) ) echo " | $site_description"; if ( $paged >= 2 || $page >= 2 ) echo ' | ' . sprintf( __( 'Page %s', 'twentyten' ), max( $paged, $page ) ); ?></title>
