...
class AuthorSpotlight_Widget extends WP_Widget {
	...
	function __construct() {	   
		...
		parent::__construct('authorspotlight', __('AuthorSpotlight'), $widget_ops, $control_ops);
	...
	function form($instance) {
		...
		<p>
		    Which social icons should be displayed for the author?<br/>
			<?php
	...
	private function _displayAuthor($args, $instance){
		...
		if((is_single() || is_page()) && $authordata->ID){			
			...
			foreach ( $this->_getIconsAsArray() as $key => $data ) {	
				...
				//while the other Social URLs come from Author meta, we added
				else if($social_url != "") {						
