	...
	jQuery(document).ready(function() { 
		...
		jQuery('#woo-sbm-menu ul#woo-sbm-menu_ul ul li').click(function(){
	    	...
	    	jQuery.post(ajax_url, data, function(response) {
				...
				jQuery('#woo-sbm-get-links-inner #template-info-name').val(name).prev().html('<span>Name:</span> '+name);
				jQuery('#woo-sbm-get-links-inner #template-info-type').val(type).prev().html('<span>Type:</span> '+type);
				jQuery('#woo-sbm-get-links-inner #template-info-slug').val(slug).prev().html('<span>Slug:</span> '+slug);
				jQuery('#woo-sbm-get-links-inner #template-info-id').val(id).prev().html('<span>ID:</span> '+id);
				if(other != ''){
					jQuery('#woo-sbm-get-links-inner #template-info-other').val(other).prev().html('<span>URL:</span> <small><a href="'+ other +'">'+ other +'</a></small>');
				} else {
					jQuery('#woo-sbm-get-links-inner #template-info-other').val('n/a').prev().html('<span>URL:</span> n/a');
         ...
         <div id="main">
         	<div id="content" class="sbm-content">
         		...
         		<div class="info-box">
         			...
         			<code>woo_sidebar</code> and replaced all <code>is_active_sidebar</code> with the new <code>woo_active_sidebar</code>. These are typically found in the 
             		...
             		<div id="woo-sbm-menu" class="postbox">
         				...
         				<li><a href="#">Pages <span>[-]</span></a>
         					<ul>
	     						...
	     						foreach ($pages as $page) {
	     							...
	     							echo '<li>' . $page->post_title . '<span>type=page&name='.$page->post_title.'&slug='.$page->post_name.'&id='. $page->ID.'&other=null</span></li>';
	     							} ?>
         					</ul>
         				</li>
         				<?php } 
         				...
         				<li><a href="#">Page Templates <span>[+]</span></a>
	     					...
	     					foreach($page_templates as $name => $template){ 
	     						...
	     						echo '<li>' . $name .'<span>type=page_template&name='.$name.'&slug='.$template.'&id=null&other=null</span></li>';
		     				}; ?>	     		
	     					</ul>
	     				</li>
         				<?php
	     				...
	     				<li><a href="#"><?php echo ucwords(str_replace('_',' ',$taxonomy)); ?> <span>[+]</span></a>
	     					...
	     					foreach($terms as $term){	     					
	     						...
	     						echo '<li>' . $term->name . '<span>type='. $taxonomy .'&name='.$term->name.'&slug='.$term->slug.'&id='.$term->term_id.'&other='.$taxonomy.'</span></li>';
	     					}?>
	     					</ul>
	     				</li>
	     					<?php
