...
if ($row1 == 0) {
	...
	$resPCd = mysql_query("SELECT * FROM profile_person_fo WHERE profile_person_fo = '" . $profileOld_name . "' ");
	$rowPCd = mysql_num_rows($resPCd);

	if ($rowPCd != 0) {
		if ($profileOld_name != '') {
			mysql_query("UPDATE profile_person_fo SET profile_person_fo='" . $profile_name . "' where profile_person_fo='$profileOld_name'");
		...
		if ($stringifyCompetitors[$c] == '0') {
			...
			if ($rowPC == 0) {
				if ($stringifyCompetitors[$c + 1] != '') {
					...
					if ($rowPPC == 0) {
						mysql_query("INSERT INTO profile (profile_name,profile_org) values('" . $stringifyCompetitors[$c + 1] . "','0')");
			...
			if ($rowPP == 0) {
				...
				if ($stringifyPeople[$c2 + 1] != '') {
					...
					if ($rowPPP == 0) {
						...
						/* mysql_query("INSERT INTO profile_person (profile_person_name,profile_id) values('".$stringifyPeople[$c2+1]."','".$companyId."')");  */
						//echo $PeopleSub[$a];
						...
						mysql_query("INSERT INTO profile_person_fo (profile_person_id,profile_person_fo,profile_person_fo_role,profile_person_fo_start,profile_person_fo_end) values('" . $profile_person_id . "','" . $profile_name . "','" . $stringifyPeople[$c2 + 2] . "','" . $stringifyPeople[$c2 + 3] . "','" . $stringifyPeople[$c2 + 4] . "')");
						$timexx = time();
						...
						$resddu = mysql_query("SELECT * FROM profile_person_fo WHERE profile_person_fo = '" . $profile_name . "' AND profile_person_id='" . $profile_person_idun . "'");
						$rowddu = mysql_num_rows($resddu);
						if ($rowddu == 0) {
							if ($profile_name != '') {
								mysql_query("INSERT INTO profile_person_fo (profile_person_id,profile_person_fo,profile_person_fo_role,profile_person_fo_start,profile_person_fo_end) values('" . $profile_person_idun . "','" . $profile_name . "','" . $stringifyPeople[$c2 + 2] . "','" . $stringifyPeople[$c2 + 3] . "','" . $stringifyPeople[$c2 + 4] . "')");
							}
						}
					}
				}
			...
			if ($rowPP == 0) {
				...
				if ($stringifyPeople[$c2 + 1] != '') {
					/* mysql_query("UPDATE profile_people SET profile_people='" . $stringifyPeople[$c2 + 1] . "',profile_people_role='" . $stringifyPeople[$c2 + 2] . "',profile_people_start='" . $stringifyPeople[$c2 + 3] . "',profile_people_end='" . $stringifyPeople[$c2 + 4] . "' where profile_people_id='$stringifyPeople[$c2]'");
					 ...
					 $sqlSPU = mysql_query("SELECT * FROM profile_people WHERE profile_people='" . $stringifyPeople[$c2 + 1] . "'");
					 $rSPU = mysql_fetch_array($sqlSPU);
					 $profile_people_id = $rSPU['profile_people_id'];
					 mysql_query("UPDATE profile_person SET profile_person_name='" . $stringifyPeople[$c2 + 1] . "' where profile_people_id='$profile_people_id'"); */
				}
			}
		}
		//mysql_query("UPDATE profile SET  ");
	...
	while ($c4 < $lengthFund) {
		if ($stringifyFund[$c4] == '0') {
			$resPF = mysql_query("SELECT * FROM profile_funding WHERE profile_funding = '" . $stringifyFund[$c4 + 1] . "' AND profile_id='" . $companyId . "'");
			$rowPF = mysql_num_rows($resPF);

			if ($rowPF == 0) {
				//Checking empty!
				if ($stringifyFund[$c4 + 1] != '') {
					mysql_query("INSERT INTO profile_funding (profile_id,profile_funding,profile_funding_round,profile_funding_asign,profile_funding_amount,profile_funding_date,profile_funding_type,profile_funding_person) values('" . $companyId . "','" . $stringifyFund[$c4 + 1] . "','" . $stringifyFund[$c4 + 2] . "','" . $stringifyFund[$c4 + 3] . "','" . $stringifyFund[$c4 + 4] . "','" . $stringifyFund[$c4 + 5] . "','" . $stringifyFund[$c4 + 6] . "','" . $stringifyFund[$c4 + 7] . "')");
					//echo $stringifyMiles[$c3+1];
				}
			}
		} else {
			...
			if ($rowPF == 0) {
				//Checking empty!
				if ($stringifyFund[$c4 + 1] != '') {
					mysql_query("UPDATE profile_funding SET profile_funding='" . $stringifyFund[$c4 + 1] . "',profile_funding_round='" . $stringifyFund[$c4 + 2] . "',profile_funding_asign='" . $stringifyFund[$c4 + 3] . "',profile_funding_amount='" . $stringifyFund[$c4 + 4] . "',profile_funding_date='" . $stringifyFund[$c4 + 5] . "',profile_funding_type='" . $stringifyFund[$c4 + 6] . "',profile_funding_person='" . $stringifyFund[$c4 + 7] . "' where profile_funding_id='$stringifyFund[$c4]'");
