...
if ($_POST) {
	...
	VALUES ('','$profile_name','$profile_description','$profile_category','$profile_homepage_url','$profile_blog_url','$profile_twitter_username','$profile_facebook_username','$profile_email','$profile_number_of_employees','$profile_founded_month','$profile_founded_day','$profile_founded_year','$profile_logo','$profile_country','$profile_screenshots','$profile_status','$profile_active','$profile_person_id','$profile_org')";
			...
			if ($rowPP == 0) {
				...
				if ($PeopleSub[$a] != '') {
					...
					if ($rowPPP == 0) {
						...
						mysql_query("INSERT INTO profile_person_fo (profile_person_id,profile_person_fo,profile_person_fo_role,profile_person_fo_start,profile_person_fo_end) values('" . $profile_person_id . "','" . $profile_name . "','" . $PeopleSub[$a + 1] . "','" . $PeopleSub[$a + 2] . "','" . $PeopleSub[$a + 3] . "')");
						$timex = time();
						...
						$resddu = mysql_query("SELECT * FROM profile_person_fo WHERE profile_person_fo = '" . $profile_name . "' AND profile_person_id='" . $profile_person_idNA . "'");
						$rowddu = mysql_num_rows($resddu);
						if ($rowddu == 0) {
							if ($profile_name != '') {
								mysql_query("INSERT INTO profile_person_fo (profile_person_id,profile_person_fo,profile_person_fo_role, 	profile_person_fo_start,profile_person_fo_end) values('" . $profile_person_idNA . "','" . $profile_name . "','" . $PeopleSub[$a + 1] . "','" . $PeopleSub[$a + 2] . "','" . $PeopleSub[$a + 3] . "')");
							}
						}
					}
				}
		...
		while ($b < $arrb) {
			...
			if ($rowPC == 0) {
				if ($CompetitorsSub[$b] != '') {
					...
					if ($rowPPC == 0) {
						mysql_query("INSERT INTO profile (profile_name,profile_org) values('" . $CompetitorsSub[$b] . "','0')");
		...
		while ($d < $arrd) {
			$resPF = mysql_query("SELECT * FROM profile_funding WHERE profile_funding = '" . $FundSub[$d] . "' and profile_id='" . $r['profile_id'] . "'");
			$rowPF = mysql_num_rows($resPF);

			if ($rowPF == 0) {
				//Checking empty!
				if ($FundSub[$d] != '') {
					mysql_query("INSERT INTO profile_funding (profile_id,profile_funding,profile_funding_round,profile_funding_asign, 	profile_funding_amount,profile_funding_date,profile_funding_type,profile_funding_person)  
		values('" . $r['profile_id'] . "','" . $FundSub[$d] . "','" . $FundSub[$d + 1] . "','" . $FundSub[$d + 2] . "','" . $FundSub[$d + 3] . "','" . $FundSub[$d + 4] . "','" . $FundSub[$d + 5] . "','" . $FundSub[$d + 6] . "')");
				}
			}
			$d = $d + 7;
		}
